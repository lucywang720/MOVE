...
class PushTKeypointsRunner(BaseLowdimRunner):
    def __init__(self,
        ...
        def env_fn():
            ...
            list_pos = np.random.randint(0, 400, size=(1,2)).tolist()[0]
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    PushTKeypointsEnv(
                        ...
                        reset_state_dict={'pos':np.random.randint(0, 400, size=(1,2)).tolist()[0]},
                        **kp_kwargs
                    ),
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                #     print(f"子进程名称: {process.name}")
                #     print(f"子进程 PID: {process.pid}")
                #     print(f"是否存活: {process.is_alive()}")
                #     print(dir(process))

                # update pbar
                pbar.update(action.shape[1])
                # print("======into set state======")
                # print(dir(env))
                # env.set_state()
            pbar.close()
