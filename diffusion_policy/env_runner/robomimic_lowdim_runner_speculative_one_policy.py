        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                if action_buffer == None or cur_time + short_T > long_T:
                    ...
                    new_log_prob = policy.log_prob(new_obs, old_naction, a_t_start=start, at_t_end=start+n_action)
                    infer_count += 1
                ...
                if not np.all(np.isfinite(exe_action)):
                    print(exe_action)
                    raise RuntimeError("Nan or Inf action")
                
                # step env
                if self.abs_action:
                    env_action = self.undo_transform_action(exe_action)
            ...
            try:
                with open(self.output_dir + '/step.txt', "r") as f:
                    data = json.load(f)
                    if not isinstance(data, list):  # 检查数据是否为列表类型
                        data = []  # 如果不是列表，则创建一个新列表
            except FileNotFoundError:
                data = []

            data.append(cnt_step)
