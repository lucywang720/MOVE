    ...
    # def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                #     if self.first_step <= 7:
                #         # print(obs_dict)
                #         action_dict = self.policy[0].predict_action(obs_dict)
                #         np_action_dict = dict_apply(action_dict,
                #             lambda x: x.detach().to('cpu').numpy())
                #         action = np_action_dict['action'][:,self.n_latency_steps:]
                #         self.first_step += 1
                #     elif self.first_step <= 11:
                #         action_dict = self.policy[2].predict_action(obs_dict)
                #         np_action_dict = dict_apply(action_dict,
                #             lambda x: x.detach().to('cpu').numpy())
                #         action = np_action_dict['action'][:,self.n_latency_steps:]
                #         self.first_step += 1
                #     else:
                #         action_dict = self.policy[-1].predict_action(obs_dict)
                #         np_action_dict = dict_apply(action_dict,
                #             lambda x: x.detach().to('cpu').numpy())
                #         action = np_action_dict['action'][:,self.n_latency_steps:] #[:,self.n_latency_steps:]                    

                #     print(action.shape, self.first_step)
                ...
                with torch.no_grad():
                    ...
                    if a == 1:
                        ...
                        np_action_dict = dict_apply(action_dict,
                            lambda x: x.detach().to('cpu').numpy())
                        action = np_action_dict['action'][:,self.n_latency_steps:self.n_latency_steps+16]
                        self.first_step += 1
                    else:
                        action_dict = self.policy[-1].predict_action(obs_dict)
                        np_action_dict = dict_apply(action_dict,
                            lambda x: x.detach().to('cpu').numpy())
                        action = np_action_dict['action'][:,self.n_latency_steps:self.n_latency_steps+32]
                        self.first_step += 1
                ...
                try:
                    with open(self.output_dir + '/step.txt', "r") as f:
                        number = int(f.read())
                except FileNotFoundError:
                    number = 0
                except ValueError:  # 处理文件中非数字内容的情况
                    number = 0
                number += 1
                with open(self.output_dir + '/step.txt', "w") as f:
                    f.write(str(number))
