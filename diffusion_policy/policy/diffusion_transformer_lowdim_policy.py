...
class DiffusionTransformerLowdimPolicy(BaseLowdimPolicy):
    def __init__(self, 
        ...
        for i in range(len(self.prob)):
            self.prob[i] = self.prob[i] / prob_sum
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # print("************************")
        # print("naction_pred: ", naction_pred, naction_pred.shape)
        # print("************************")
        action_pred = self.normalizer['action'].unnormalize(naction_pred)
        # print("action_pred: ", action_pred, action_pred.shape)
        # print("************************")
    ...
    def compute_loss(self, batch):
        ...
        # print("***", nbatch['obs'].shape, action.shape, self.n_obs_steps)
        # exit(0)
        ...
        #     chosen_length = random.choices(mask_lengths, weights=self.prob, k=1)[0]
        ...
        # print(pred.shape, target.shape, loss_mask.shape)
        masked_pred = pred * loss_mask
        masked_target = target * loss_mask

        # masked_pred = pred[:, :34, :]
        # masked_target = target[:, :34, :]


        loss = F.mse_loss(masked_pred, masked_target, reduction='none')
        loss = loss * loss_mask.type(loss.dtype)
        # print("loss and mask shape", loss.shape, loss_mask.shape)
