...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    ...
    def __init__(
            ...
            action_rot_scale=1.,
            random_init=True,
            obj_hand_rotate=False
    ):
        super().__init__(model_name, frame_skip=frame_skip)
        self.random_init = random_init
        self.obj_hand_rotate = obj_hand_rotate
    ...
    def set_xyz_action(self, action):
        ...
        if not (quat is None):
            ...
            # self.data.set_mocap_pos('mocap', new_mocap_pos)
            self.data.set_mocap_quat('mocap', compute_ee_quat(quat))
            self.data.set_mocap_pos('mocap', new_mocap_pos)
    ...
    def set_camera_pos(self, v_theta_cam=0, v_r_camera=0, v_h_camera=0, THETA_RANGE=(0, np.pi)): # 
        if hasattr(self, 'CAMERA_DISABLE') and self.CAMERA_DISABLE:
            return
        assert v_r_camera == 0, "SawyerXYZEnv does not support radial camera movement"
        assert v_h_camera == 0, "SawyerXYZEnv does not support height camera movement"
        theta = self.camera_controller.theta
        r = self.camera_controller.r
        h = self.camera_controller.z
        theta = np.clip(theta+v_theta_cam, *THETA_RANGE)
    ...
    def _set_obj_xyz_move(self, pos, target=None):
        qpos = self.data.qpos.flat.copy()
        qvel = self.data.qvel.flat.copy()
        qpos[9:12] = pos.copy()
        qvel[9:15] = 0
        self.set_state(qpos, qvel, forward=False, target_pos=target)
    ...
    def _get_curr_obs_combined_no_goal(self):
        ...
        # if self.obj_hand_rotate:
        #     print(self.data.get_body_xquat('hand'))
        #     quat_hand = self.data.get_body_xquat('hand')
        # else:
        #     quat_hand = None
        ...
        if self.isV2:
            ...
            # print(obj_quat_split, obj_pos_split, obs_obj_padded, len(obj_pos), len(obj_quat))
            # for pos, quat in zip(obj_pos_split, obj_quat_split):
                # print(pos, quat) 

            if len(obj_pos_split) == len(obj_quat_split):
                ...
                # print(min(len(obj_pos_split),len(obj_quat_split))*7)              

                obs_obj_padded[:min(len(obj_pos_split),len(obj_quat_split))*7] = np.hstack([
                    np.hstack((pos, quat))
                    for pos, quat in zip(obj_pos_split, obj_quat_split)
                ])
                if len(obj_pos_split) > len(obj_quat_split):
                    obs_obj_padded[min(len(obj_pos_split),len(obj_quat_split))*7:min(len(obj_pos_split),len(obj_quat_split))*7+(len(obj_pos_split)-len(obj_quat_split))*3] = np.hstack(obj_pos_split[len(obj_pos_split)-len(obj_quat_split):])
                elif len(obj_pos_split) < len(obj_quat_split):
                    obs_obj_padded[min(len(obj_pos_split),len(obj_quat_split))*7:min(len(obj_pos_split),len(obj_quat_split))*7+(len(obj_quat_split)-len(obj_pos_split))*4] = np.hstack(obj_quat_split[len(obj_quat_split)-len(obj_pos_split):])

                # print(obs_obj_padded)
                # exit(0)
    ...
    def step(self, action):
        ...
        #     rotation_matrix = self.sim.data.get_geom_xmat('objGeom')
        #     euler_angles = Rotation.from_matrix(rotation_matrix).as_euler('xyz', degrees=True)
        #     z_tilt = abs(euler_angles[2])
        #     y_tilt = abs(euler_angles[1])
        #     x_tilt = abs(euler_angles[0])
        #     # if ((0 <= z_tilt <= 6) or (174 <= z_tilt <= 180)) and ((0 <= x_tilt <= 6) or (174 <= x_tilt <= 180)) and ((0 <= y_tilt <= 7) or (173 <= y_tilt <= 180)):
        #     if ((0 <= z_tilt <= 6) or (174 <= z_tilt <= 180)) and ((0 <= x_tilt <= 6) or (174 <= x_tilt <= 180)):

        #         self.correct_render = True
        #         with open('./height.txt', 'a') as f:
        #             f.write(str(self.obj_init_pos) + '\n')
        #     else:
        #         self.correct_render = False
        #     print("self.correct_render: ", self.correct_render, x_tilt, y_tilt, z_tilt, self.obj_init_pos)
        ...
        # print(self.sim.data.site_xpos[0])
        # print("=============")
    ...
    def _reset_hand_grip(self, steps=50):
        for _ in range(steps):
            # self.data.set_mocap_pos('mocap', self.hand_init_pos)
            # self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
            self.do_simulation([-1, 1], self.frame_skip)
