...
class SawyerDrawerOpenRotateHandV2Policy(Policy):
    @staticmethod
    @assert_fully_parsed
    def _parse_obs(obs):
        return {
    ...
    def get_action(self, obs):
        ...
        # pos_drwr = np.array([o_d['drwr_pos'][0]+0.16v_*math.sin(2*math.asin(o_d['drwr_quat'][-1])), o_d['drwr_pos'][1]-0.16*math.cos(2*math.asin(o_d['drwr_quat'][-1]))+0.16, pos_drwr[2]])
        pos_drwr = np.array([o_d['drwr_pos'][0]+0.16*math.sin(2*math.asin(o_d['drwr_quat'][-1])), o_d['drwr_pos'][1]-0.16*math.cos(2*math.asin(o_d['drwr_quat'][-1]))+0.16, o_d['drwr_pos'][2]])

        # print(o_d['v_move'], o_d['d_move'])
        pos_drwr += o_d['v_move'] * o_d['d_move'] * 2
