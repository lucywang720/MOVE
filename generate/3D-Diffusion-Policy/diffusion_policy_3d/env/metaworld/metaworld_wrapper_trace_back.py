...
# (-0.2, 0.1) (0.5, 0.8)
eval_pos_x = [-0.18+0.01*i for i in range(0,27)]
eval_pos_y = [0.52+0.01*i for i in range(0,27)]

# 5*5
# train_pos_x = [-0.15, -0.1, -0.05, 0, 0.05]
# train_pos_y = [0.55, 0.6, 0.65, 0.7, 0.75]

# 3*3
train_pos_x = [-0.15, -0.05, 0.05]
train_pos_y = [0.55, 0.65, 0.75]

# 4*4
# train_pos_x = [-0.15, -0.08, -0.02, 0.05]
# train_pos_y = [0.55, 0.62, 0.68, 0.75]

# 10*9
# train_pos_x = [-0.2+0.3/11*(i+1) for i in range(0,10)]
# train_pos_y = [0.5+0.3/10*(i+1) for i in range(0,9)]
...
class MetaWorldTracebackEnv(gym.Env):
    ...
    def __init__(self, task_name, device="cuda:0", 
        ...
        super(MetaWorldTracebackEnv, self).__init__()
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        #     self.env.set_new_obj(state=[train_pos_x[min(int(self.trajectory_cnt//len(train_pos_x)), len(train_pos_x)-1)], train_pos_y[self.trajectory_cnt%len(train_pos_x)], 0.02])
        #     self.move_step += 1
        #     self.reverse_point_list = self.env.trace_back([train_pos_x[min(int(self.trajectory_cnt//len(train_pos_x)), len(train_pos_x)-1)], train_pos_y[self.trajectory_cnt%len(train_pos_x)], 0.02], self.v, self.d)
        ...
        if self.cur_step < 35 and move_collect and self.v:
            self.env.set_new_obj(state=self.reverse_point_list[self.cur_step]) #self.d_tmp
            self.move_step += 1
    ...
    def reset(self):
        ...
        self.reset_obj_cnt = 0
        self.grasp_cnt = 0
        self.reverse_point_list = []

        if self.cur_step == 0:
            # self.env.set_new_obj(state=[train_pos_x[min(int(self.trajectory_cnt//5), 4)], train_pos_y[min(self.trajectory_cnt%5, 4)], 0.02])
            print(self.trajectory_cnt)
            # tmp_x = train_pos_x[min(int((self.trajectory_cnt%(len(train_pos_x)*len(train_pos_y)))//len(train_pos_x)), len(train_pos_x)-1)]+0.001*np.random.randn()
            # tmp_y = train_pos_y[self.trajectory_cnt%len(train_pos_y)]+0.001*np.random.randn()
            tmp_x = train_pos_x[min(int((self.trajectory_cnt%(len(train_pos_x)*len(train_pos_y)))//len(train_pos_x)), len(train_pos_x)-1)]
            tmp_y = train_pos_y[self.trajectory_cnt%len(train_pos_y)]
            ...
            self.move_step += 1
            self.reverse_point_list = self.env.trace_back([tmp_x, tmp_y, 0.02], self.v, self.d)
