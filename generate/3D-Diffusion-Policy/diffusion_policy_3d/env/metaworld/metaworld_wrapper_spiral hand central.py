...
insert_num = 36
begin = [0, 2.8, 4.6, 6.4, 7.3, 8.2, 9.1, 10, 10.9, 11.8, 12.7, 13.6, 14.5, 15, 15.4, 15.7, 16]
leng = len(begin) - 1
spiral_theta = np.concatenate([np.linspace(np.pi*begin[i], np.pi*begin[i+1], insert_num)[:-1] for i in range(0,leng)])
spiral_theta = np.concatenate([spiral_theta, np.array([16*np.pi])])
...
for i in range(len(begin)-1):
    for j in range(0, 5):
        eval_points_filter.append(spiral_pos_theta(np.pi*(begin[i]+(begin[i+1]-begin[i])/5*j)))

eval_points_filter.append(spiral_pos_theta(np.pi*16))
