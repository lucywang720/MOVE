...
class MetaWorldTableEnv(gym.Env):
    metadata = {"render.modes": ["rgb_array"], "video.frames_per_second": 10}

    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform', velocity_move=0.0, move_step=25, velocity_height=0.0, velocity_camera=0.0, retarget=False
                 ):
        super(MetaWorldTableEnv, self).__init__()
        ...
        # self.v = np.random.beta(a=2, b=5, size=1)[0] * 0.03
        self.v_move = np.random.beta(a=2, b=5, size=1)[0] * self.velocity_move
        self.v_desk = (np.random.beta(a=2, b=5, size=1)[0] * self.velocity_height) * np.random.choice([-1,1])
        # self.v = 0
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        if self.cur_step < self.move_step and move_collect:
            if self.env.change_height and not env_info['grasp_success'] and action[-1] == 0:
                # self.v_desk = self.env.set_desk_height(v_desk=self.v_desk)
                self.d, self.v_desk = self.env.set_obj_and_desk_height(v=self.v_move, d=self.d, v_desk=self.v_desk)

                # print(raw_state)
                raw_state[11] = self.v_desk
                raw_state[12:15] = self.v_move

    
        elif (not self.retargeted) and env_info['grasp_success']:
        ...
        # if self.env.correct_render == False:
        #     done = True
        #     reward = 0
