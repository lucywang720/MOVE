...
class MetaWorldEnv(gym.Env):
    ...
    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=0, sampling_type=''
                 ):
    ...
    def reset_sampling(self, sampling='uniform'):
        lower, upper = self.retarget_low, self.retarget_high  # 采样区间
        n_samples = 1       # 采样数量
    ...
    def step(self, action: np.array, collect=False):
        ...
        if env_info['grasp_success'] and self.grasp_success_step == 0:
            self.grasp_success_step = self.cur_step
        ...
        # print(env_info)
        # print(self.task_name, env_info['success'])

        if 'drawer' in self.task_name:
            if env_info['success'] and collect:# and self.already_change:
                self.success_reward_step += 1

                if self.success_reward_step > 9:
                    done = True            

        elif reward == 10 and collect:# and self.already_change:
            self.success_reward_step += 1

            if self.success_reward_step > 9:
                done = True

        # print(done, self.cur_step, reward)
