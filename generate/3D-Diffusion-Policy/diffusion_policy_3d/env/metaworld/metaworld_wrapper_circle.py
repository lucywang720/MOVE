...
def sample_circle_points(center=[-0.05, 0.65], radius=0.15, num_points=20):
    """
    在圆上均匀采样指定数量的点
    
    参数:
    center -- 圆心坐标 [x, y] (默认: [-0.05, 0.65])
    radius -- 圆半径 (默认: 0.15)
    num_points -- 采样点数量 (默认: 20)
    
    返回:
    包含20个点坐标的列表，每个点为[x, y]
    """
    h, k = center
...
class MetaWorldCircleEnv(gym.Env):
    metadata = {"render.modes": ["rgb_array"], "video.frames_per_second": 10}

    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform', velocity=0.0, velocity_camera=0.0, velocity_obj=0.0, r=0.15
                 ):
        super(MetaWorldCircleEnv, self).__init__()
        ...
        self.circle_grasp_points = sample_circle_points(radius=self.r)

        random.shuffle(self.circle_grasp_points)
    ...
    def reset(self):
        ...
        if self.cur_step == 0:
            ...
            tmp_x = self.circle_grasp_points[self.trajectory_cnt%20][0]+self.noise_x
            tmp_y = self.circle_grasp_points[self.trajectory_cnt%20][1]+self.noise_y

            if self.v != 0:
                self.reverse_point_list = self.env.trace_back_circle([tmp_x, tmp_y, 0.02], self.v, self.d, self.r)
            else:
                self.reverse_point_list = [[tmp_x, tmp_y, 0.02] for i in range(35)]

            # tmp_x = self.circle_grasp_points[self.cur_step][0]+self.noise_x
            # tmp_y = self.circle_grasp_points[self.cur_step][1]+self.noise_y
