...
class MetaWorldAsseMoveEnv(gym.Env):
    metadata = {"render.modes": ["rgb_array"], "video.frames_per_second": 10}

    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform', velocity_move=0.0, velocity_height=0.0, velocity_rotate=0.0
                 ):
        super(MetaWorldAsseMoveEnv, self).__init__()

        print("into MetaWorldAsseMoveEnv")
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        if not self.grasp_success:
            self.grasp_success = env_info['grasp_success']

        if self.cur_step < self.env.retarget_step and move_collect:
            # print(env_info['grasp_success'])
            if self.env.retarget and self.grasp_success:
                print(self.cur_step)
                tmp_d = self.env.set_new_target(v=self.v, d=self.d)
                if not (tmp_d is None):
                    self.d = tmp_d

        if self.cur_step < self.env.move_obj_step and self.env.move_object and not env_info['grasp_success']:
