...
class MetaWorldRotateEnv(gym.Env):
    ...
    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform', velocity_rotate=0.0, velocity_move=0.0, move_step=35
                 ):
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        # print(self.env.data.qpos)
        # if self.cur_step < 10:
        #     raw_state, reward, done, env_info = self.env.step(np.zeros(4))
        #     # print(action)
        #     # exit(0)
        # else:
        raw_state, reward, done, env_info = self.env.step(action)
        ...
        if self.cur_step < self.move_step and move_collect:
            ...
            #         self.d = tmp_d
            # if self.env.move_object and not env_info['grasp_success']:
            #     self.d = self.env.set_new_obj(v=self.v, d=self.d)
            # if self.env.rotate_object:
        ...
        # raw_state[4:7] = self.env._get_pos_objects()
        # raw_state[7:11] = self.env.sim.model.body_quat[self.env.sim.model.geom_name2id('WrenchHandle')]
