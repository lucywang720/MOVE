...
class MetaWorldEnv(gym.Env):
    ...
    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform'
                 ):
        ...
        self.reset_obj_num = 1 #3
        self.reset_obj_cnt = 0
        self.grasp_cnt = 0
        self.retarget_step_list = []

        self.v = np.random.beta(a=2, b=5, size=1)[0] * 0.02
    ...
    # def step(self, action: np.array, collect=False):
    #     # print(self.env.data.qpos)
    #     raw_state, reward, done, env_info = self.env.step(action)
    #     self.cur_step += 1  # 更新总步数计数器
    #     # self.all_trajectory_step += 1

    #     # print(env_info['grasp_success'])

    #     # ===================== Retarget 逻辑控制 =====================
    #     # 1. 成功抓取且需要安排新的 retarget
    #     # if not env_info['grasp_success'] and self.cur_step == self.reset_obj_step:
    #     #     self.env.set_new_obj()

    #     env_info['set_obj'] = False
    #     # print(self.env.get_body_com('leftpad'), self.env.get_body_com('rightpad'))
    #     # print(dir(self.env.data))
    #     # self.env.data.set_body_com('left_pad', [0.00612921, 0.64705053, 0.19388809])

    #     # if self.cur_step == self.reset_obj_step:
    #     #     self.env.set_new_obj()
    #     #     self.env._reset_hand_grip()
    #     #     env_info['set_obj'] = True
    #     #     self.retarget_step_list.append(self.cur_step)

    #     #     print("reset!")

    #     if collect and env_info['grasp_success']:
    #         if not self.waiting_for_retarget and self.remaining_retargets > 0:
    #             # 生成新的 retarget 间隔
    #             print(self.retarget_high, self.retarget_low)
    #             retarget_interval = self.reset_sampling(self.sampling_type, low=self.retarget_low, high=self.retarget_high)
    #             self.target_step = self.cur_step + retarget_interval
    #             self.remaining_retargets -= 1
    #             self.waiting_for_retarget = True
                
    #             print(f"▶ Retarget scheduled: step {self.cur_step} → {self.target_step} "
    #                 f"(interval {retarget_interval}), remaining: {self.remaining_retargets}")

    #     # 2. 到达目标步数时执行 retarget
    #     if self.waiting_for_retarget and self.cur_step >= self.target_step:
    #         self.env.set_new_target()
    #         self.waiting_for_retarget = False
    #         print(f"✓ Retarget executed at step {self.cur_step}")
            
    #         # 重置成功计数器（如果需要保持连续成功）
    #         self.success_counter = 0  
    #         self.retarget_step_list.append(self.cur_step)

    #     # print("STEP: ", self.cur_step, env_info, "reward: ", reward, self.success_reward_step)
    ...
    #     #     if self.success_counter >= 3:  # 连续成功3次则提前终止
    #     #         done = True
    #     #         reward += 10  # 额外奖励
    #     # else:
    #     #     self.success_counter = 0
    #     # if reward == 10 and collect and self.remaining_retargets==0 and (not self.waiting_for_retarget) and env_info['grasp_success']:
    #     #     self.success_reward_step += 1
    #     if reward == 10 and collect and self.remaining_retargets==0 and (not self.waiting_for_retarget):
    #         self.success_reward_step += 1
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        # if self.cur_step < 50 and not env_info['grasp_success'] and move_collect:

        #     self.env.set_new_obj(v=self.v, d=self.d)
        #     # self.env._reset_hand_grip()
        #     env_info['set_obj'] = True
        #     self.v = self.v * 0.9
        #     # self.retarget_step_list.append(self.cur_step)

        # if env_info['grasp_success'] and self.grasp_cnt == 4 and self.reset_obj_cnt < self.reset_obj_num and move_collect:
        if self.grasp_cnt == 6 and self.reset_obj_cnt < self.reset_obj_num and move_collect:
            self.env.set_new_obj(state=1)
            self.env._reset_hand_grip()
            self.env.set_new_target()
            env_info['set_obj'] = True
            self.retarget_step_list.append(self.cur_step)
            self.grasp_cnt = 0
            self.reset_obj_cnt += 1
            print("reset!", self.reset_obj_cnt, self.reset_obj_num, self.cur_step)
        elif env_info['grasp_success']:
            # print("!", self.cur_step)
            self.grasp_cnt += 1

        # print("-", self.cur_step, env_info)

        # if self.cur_step < self.reset_obj_step and self.cur_step % 2 == 0:
        #     self.env.set_new_obj()
        ...
        # if reward == 10 and collect and self.remaining_retargets==0 and (not self.waiting_for_retarget) and env_info['grasp_success']:
        #     self.success_reward_step += 1
        print(reward, collect, self.remaining_retargets==0, (not self.waiting_for_retarget), (self.reset_obj_cnt == self.reset_obj_num and move_collect))
        # if reward == 10 and collect and self.remaining_retargets==0 and (not self.waiting_for_retarget) and (self.reset_obj_cnt == self.reset_obj_num and move_collect):
    ...
    def reset(self):
        ...
        self.success_counter = 0
        self.success_reward_step = 0
        self.reset_obj_step = self.reset_sampling(self.sampling_type, 35, 50)
        self.retarget_step_list = []
