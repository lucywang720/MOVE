...
class MetaWorldRelocateEnv(gym.Env):
    metadata = {"render.modes": ["rgb_array"], "video.frames_per_second": 10}

    def __init__(self, task_name, device="cuda:0", 
                 use_point_crop=True,
                 num_points=1024,
                 retarget_low=10, retarget_high=80, max_retargets=2, sampling_type='uniform', velocity=0.0, velocity_camera=0.0
                 ):
        super(MetaWorldRelocateEnv, self).__init__()
        ...
        self.v = np.random.beta(a=2, b=5, size=1)[0] * self.velocity
        # self.v = 0
        ...
        self.new_obj_total = 4
        self.new_target_total = 4

        self.now_obj_total = 0
        self.now_target_total = 0
    ...
    def step(self, action: np.array, collect=False, move_collect=False):
        ...
        # print(np.linalg.norm(self.env.obj_init_pos-self.env.tcp_center), np.linalg.norm(self.env._target_pos-self.env.tcp_center), self.now_obj_total, self.now_target_total)

        # if np.linalg.norm(self.env.obj_init_pos-self.env.tcp_center) < 0.2 and self.now_obj_total < self.new_obj_total:
        # if np.linalg.norm(self.env.obj_init_pos-self.env.tcp_center) < 0.2 and self.now_obj_total < self.new_obj_total:# and (self.cur_step-self.change_step) > 5:
        if np.linalg.norm(self.env._get_pos_objects()-self.env.tcp_center) < 0.2 and self.now_obj_total < self.new_obj_total:
            print(np.linalg.norm(self.env.obj_init_pos-self.env.tcp_center), np.linalg.norm(self.env._target_pos-self.env.tcp_center), self.now_obj_total, self.now_target_total)
            self.env.set_new_obj()
            self.now_obj_total += 1
            self.change_step = self.cur_step
        if np.linalg.norm(self.env._target_pos-self.env.tcp_center) < 0.2 and self.now_target_total < self.new_target_total:
            self.env.set_new_target()
            self.now_target_total += 1
        ...
        if 'drawer' in self.task_name or 'box' in self.task_name:
    ...
    def reset(self):
        ...
        self.new_obj_total = np.random.randint(0,5)
        self.new_target_total = np.random.randint(0,5)
